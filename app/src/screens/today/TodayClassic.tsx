...
export function TodayClassic({ s, derived, actions }: ScreenProps) {
  return (
    <div className="pci-screen pci-topbar scrolls" style={{ padding: '0 24px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--color-accent)', fontFeatureSettings: "'tnum'" }}>
        <span>Tuesday · 2 Sept</span><span>Day {derived.dayNo}</span>
      </div>
      <h1 style={{ margin: '14px 0 4px', fontWeight: 400, fontSize: 40, lineHeight: 1.02 }}>Good morning, {derived.name}.</h1>
      <p style={{ margin: '0 0 20px', fontSize: 14, color: 'var(--color-neutral-700)' }}>{TODAY_PRACTICE.chapter}</p>
      ...
      <div style={{ marginTop: 20, border: '1px solid var(--color-divider)', borderRadius: 'var(--radius-md)', padding: '18px 18px 16px' }}>
        <div style={{ fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--color-accent)' }}>
          Today's practice · {TODAY_PRACTICE.minutes} min
        </div>
        <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: 24, lineHeight: 1.15, margin: '8px 0 8px' }}>{TODAY_PRACTICE.title}</div>
        <p style={{ margin: '0 0 14px', fontSize: 13.5, color: 'var(--color-neutral-700)', textAlign: 'justify', hyphens: 'auto' }}>{TODAY_PRACTICE.blurb}</p>
        {s.practiceDone ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 0 4px', fontSize: 13, color: 'var(--color-accent-700)' }}>
            <CheckIcon style={{ width: 16, height: 16 }} />Completed at {s.doneAt} — well done.
          </div>
        ) : (
      ...
      <div style={{ marginTop: 22 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
          <button className="btn btn-ghost" onClick={actions.saveReflection} disabled={!s.reflection.trim()} style={{ minHeight: 40 }}>
      ...
      <button className="pci-row" onClick={actions.toNote} style={{ marginTop: 8, borderTop: '1px solid var(--color-divider)', minHeight: 64 }}>
        <span>
          <span style={{ display: 'block', fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--color-accent)' }}>From your coach · this week</span>
          <span style={{ display: 'block', fontFamily: 'var(--font-heading)', fontSize: 18, marginTop: 2 }}>On the difference between looking and seeing</span>
        </span>
      ...
      <div style={{ paddingTop: 18 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--color-neutral-700)', fontFeatureSettings: "'tnum'" }}>
            <span>Chapter II · Attention</span><span>{derived.chapterDone} of 14</span>
          </div>
